...
.DESCRIPTION
    ...
    3. Securely and efficiently copies the same files to a source destination using SSH and Rsync via WSL.
...
try {
    ...
    # --- STEP 2: Copying to destination via Rsync (WSL) ---
    Write-Host "`n[Step 2/2] Copying files to destination via Rsync (using WSL)..." -ForegroundColor Cyan
    ...
    # Convert the Windows source path to a WSL path (e.g., C:\Users -> /mnt/c/Users)
    $wslSourcePath = (wsl wslpath -u $localSourcePath)

    # Construct the remote destination path
    $remoteDestination = "$($user)@$($deviceHostName):'$($destinationPath)/'"

    # Construct the full rsync command to be executed within WSL
    $rsyncCommand = "rsync -avz --progress -e ssh `"$wslSourcePath/`" `"$remoteDestination`""
    
    Write-Host "Running WSL Rsync command: $rsyncCommand" -ForegroundColor Yellow
    
    # Execute the command using wsl
    wsl -- $rsyncCommand

    if ($LASTEXITCODE -ne 0) {
        Write-Error "Rsync file transfer failed. Check the output above for details."
        exit
    }
    Write-Host "Rsync transfer complete." -ForegroundColor Green
